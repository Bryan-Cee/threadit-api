...
module.exports = {
  ...
  login: async (_, { email, password }, { client }) => {
    const _user = await client.getUserByEmail(email);
    const userInputError = {};
    ...
    const user = await client.getUserProfile(_user.userId);
    const token = jwt.sign({
      data: { user }
    }, process.env.SECRET_KEY, {
      expiresIn: "3hr"
  ...
  resetRequest: async (_, { email }, { client, req }) => {
    ...
    return "The password reset link has been sent to your email. It expires in an hour";
  },
  resetPassword: async (_, { password }, { client, req }) => {
    ...
    const { data: { userId } } = jwt.decode(token);
    const user = await client.getUserAccountById(userId);
    const secretKey = `${user.password}-${+user.createdAt}`;
